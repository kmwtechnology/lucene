...
/**
 * Tests the {@link SimilarityBase}-based Similarities. Contains unit tests and integration tests
 * for all Similarities and correctness tests for a select few.
 *
 * <p>This class maintains a list of {@code SimilarityBase} subclasses. Each test case performs its
 ...
 *
 * <p>Unit testing is performed by constructing statistics manually and calling the {@link
 * SimilarityBase#score(BasicStats, double, double)} method of the Similarities. The statistics
 * represent corner cases of corpus distributions.
 *
 * <p>For the integration tests, a small (8-document) collection is indexed. The tests verify that
 * for a specific query, all relevant documents are returned in the correct order. The collection
 * consists of two poems of English poet <a
 * href="http://en.wikipedia.org/wiki/William_blake">William Blake</a>.
 *
 * <p>Note: the list of Similarities is maintained by hand. If a new Similarity is added to the
 * {@code org.apache.lucene.search.similarities} package, the list should be updated accordingly.
 *
 * <p>In the correctness tests, the score is verified against the result of manual computation.
...
public class TestSimilarityBase extends LuceneTestCase {
  ...
  public void testLMJelinekMercer() throws IOException {
    ...
    float gold =
        (float) (Math.log(p / (0.1f * (TOTAL_TERM_FREQ + 1) / (NUMBER_OF_FIELD_TOKENS + 1.0f))));
  ...
  public void testIn2() throws IOException {
    ...
    float tfn =
        (float)
            (FREQ
                * SimilarityBase.log2( // 8.1894750101
                    1 + AVG_FIELD_LENGTH / DOC_LEN));
    float gold =
        (float)
            (tfn
                * SimilarityBase.log2( // 26.7459577898
  ...
  public void testIFB() throws IOException {
    ...
    float IF =
        (float)
            (FREQ
                * SimilarityBase.log2( // 8.97759389642
  ...
  /**
   * The generic test core called by all correctness test methods. It calls the {@link
   * SimilarityBase#score(BasicStats, double, double)} method of all Similarities in {@link #sims}
   * and compares the score against the manually computed {@code gold}.
   */
