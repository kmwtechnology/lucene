...
public class TestCustomSearcherSort extends LuceneTestCase {
  ...
  public void testFieldSortCustomSearcher() throws Exception {
    ...
    Sort custSort =
        new Sort(new SortField("publicationDate_", SortField.Type.STRING), SortField.FIELD_SCORE);
  ...
  public static class CustomSearcher extends IndexSearcher {
    ...
    @Override
    public TopFieldDocs search(Query query, int nDocs, Sort sort) throws IOException {
    ...
    @Override
    public TopDocs search(Query query, int nDocs) throws IOException {
      BooleanQuery.Builder bq = new BooleanQuery.Builder();
      bq.add(query, BooleanClause.Occur.MUST);
      bq.add(
          new TermQuery(new QueryTerm("mandant", Integer.toString(switcher), 0)),
  ...
  private static class RandomGen {
    ...
    private String getLuceneDate() {
      return DateTools.timeToString(
          base.getTimeInMillis() + random.nextInt() - Integer.MIN_VALUE, DateTools.Resolution.DAY);
    }
  }
}
